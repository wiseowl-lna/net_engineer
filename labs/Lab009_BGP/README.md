...
Так же на рисунке видно кто является соседями маршрутизатора R22 по протоколу BGP, в каких AREA находятся соседи, сколько сообщений было отправлено соседям и сколько от их получено (эта информация может пригодиться про поиске неисправностей).
...
В данном выводе виду, что R21 пытается открыть сессию с R24, но так как R24 не запущен или не исправлен кабель между этими маршрутизаторами - отображается состояние Active.

Настрою протокол BGP на остальных маршрутизаторах, согласно условию лабораторной работы.
