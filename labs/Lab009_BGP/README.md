...
После настройки маршрутизатора R22 ввела команду **__sh ip bgp summary__** (вывод на рис.2). На рисунке видим, что взаимодействие с соседями (пиринг) 10.0.0.1 и 10.0.0.3 по протоколу BGP, находится в состоянии ожидания (параметр State/PfxRcd в состоянии Idle). Как только я настрою соседей и произойдет соединение, значение IDLE заменится на 1 (Если ATL использует только один префикс с TPA 1).
...
2. Настроила eBGP на роутерах офиса Москва:
