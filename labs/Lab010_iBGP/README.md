...
Так же на рисунке видно кто является соседями маршрутизаторов R14 и R15 по протоколу BGP, в каких AS находятся соседи, сколько сообщений было отправлено соседям и сколько от них получено (эта информация может пригодиться при поиске неисправностей).
...
Для управления исходящим трафиком BGP использую атрибут Local preference. Этот атрибут передается только в пределах одной автономной системы. На маршрутизаторах Cisco по умолчанию значение атрибута — 100, по этому буду использовать значение, например, 500 из возможного диапазона от 0 до 4294967295, так как выбирается та точка выхода, у которой значение атрибута больше.

Для того, что бы изменить значения по умолчанию, можно использовать два варианта: команду **_bgp default local-preference_** или route-map. Я выбрала первый вариант.
...
Проверю проходжение трафика после данной настройки. Запустила на маршрутизаторе R14 команду **_sh ip route_**. Результат вывода на рисунке 4а (вывод команды не полный).

Рисунок 4а.

![](Sh_ip_route_1_R14.png)
![](Sh_ip_route_2_R14.png)

4. Настроила маршрутизатор R18 в офисе Санкт Петербург так, чтобы трафик распределялся по двум линкам одновременно. Для этого использовала команду **_maximum-paths \<n>_** указав количество разрешенных маршрутов.

**_Маршрутизатор R18:_**
...
Для проверки ввела команды **_sh ip bgp summary_** для IPv4 и **_sh bgp ipv6 unicast_** для IPv6 на R2 (рис.5).
...
Как видно из рисунков 3 и 4, связность между офисами Москва и Санкт Петербург существует (реализовано в предыдущей лабораторной работе). Теперь необходимо, что бы о сетях Лабытнанги и Чокурдаха узнал мир. Для этого анансирую эти сети на маршрутизаторах R25 и R26. Привела пример настройки R25.

**_Маршрутизатор R25:_**
...
---------------------------------------------------------------
    ...
    router bgp 520
     address-family ipv4
      ...
      etwork 192.168.4.0 mask 255.255.255.0
      exit
     address-family ipv6
...
Осталось провести проверки IP доступности между офисами. 
...
_**ping с VPC1 (Москва) на VPC8 (Санкт Петербург), VPC31 (Чокурдах), R27 (Лабытнанги)_** - рисунок 6.

Рисунок 6.

![](Ping_VPC1.png)

**_ping с VPC (Санкт Петербург) на VPC30 (Чокурдах) и R27 (Лабытнанги)_** рисунок 7.

Рисунок 7.

![](Ping_VPC.png)

**_ping с R27 (Лабытнанги) на VPC31 (Чокурдах)_** рисунок 8.
...
На рис.9 размещены используемые сети, IPv4 и IPv6 адреса маршрутизаторов.
