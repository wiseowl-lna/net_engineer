...
1. В моем случае между маршрутизаторами R14 и R15 нет прямой физической связи и для этой ситуации, как нельзя лучше, подходит протокол iBGP, который устанавливает сессию  несмотря на то, что на промежуточном маршрутизаторе BGP не включен. 
Подняла протокол iBGP на роутерах R14 и R15, указала соседей через IP адреса LoopBack, активировала в address-family IPV4 и IPv6. Так же, для выполнения пункта 3 лабораторной работы, воспользовалась командой **_next-hop-self_**, которая помогла указать через какой маршрутизатор должен идти трафик.
...
---------------------------------------------------------------
    ...
    router bgp 1001
     ...
     address-family ipv4
      ...
      ! Объявила соседу (192.168.1.14) о том, что следующим хопом являюсь "Я" - маршрутизатор R15.
...
После настройки маршрутизатора R22 ввела команду **_sh ip bgp summary_** (вывод на рис.2). На рисунке вижу, что взаимодействие с соседями (пиринг) 10.0.0.1 и 10.0.0.3 по протоколу BGP, находится в состоянии ожидания (параметр State/PfxRcd в состоянии Idle). Как только я настрою соседей и произойдет соединение, значение IDLE заменится на количество принятых префиксов.
...
---------------------------------------------------------------
    ...
    conf t
    !
    ! Подняла протокол BGP, указала соседей, активировала в address-family
    router bgp 1001
...
Теперь посмотрю, появились ли какие-либо изменения после ввода команды **_sh ip bgp summary_** на R14  и R22 (рис.3).
