...
### Ход выполнения:
    Для выполнения лабораторной работы использовался эмулятор EVE-NG, терминальный клиент SuperPuTTY.

#### **_I. Документирование адресного пространства для лабораторного стенда._**

   *_Используемые сети:_*

10.0.0.0/8 - пользовательские сети.

100.100.100.0/24 - "Белые адреса" для эмуляции сети Интернет и настройки NAT.

172.0.0.0/16 - сети провайдеров.
...
#### a. Таблица выделенных подсетей.
    ...
    Таблица 1.
    |----------------|-------|-------------------|-------------------|---------------------------------------|
    |  Расположение  |  AS   |      IPv4 сеть    | Родительская сеть |      Описание                         |
    |----------------|-------|-------------------|-------------------|---------------------------------------|
    | Большие Боты   | 12389 | 172.1.0.0/24      |  172.1.0.0/16     | Loopback's                            |
    |                | 12389 | 172.1.1.0/24      |  172.1.0.0/16     | Link's                                |
    |----------------|-------|-------------------|-------------------|---------------------------------------|
    | Малые Боты     | 65666 | 172.6.0.0/24      |  172.6.0.0/16     | Loopback's                            |
    |                | 65666 | 172.6.1.0/24      |  172.6.0.0/16     | Link's                                |
    |----------------|-------|-------------------|-------------------|---------------------------------------|
    | Старые Боты    | 65555 | 172.5.0.0/24      |  172.5.0.0/16     | Loopback's                            |
    |                | 65555 | 172.5.0.0/24      |  172.5.0.0/16     | Link's                                |
    |----------------|-------|-------------------|-------------------|---------------------------------------|
    | Весёлая Жизнь  | 4     | 10.4.0.0/24       |  10.4.0.0/16      | Loopback's                            |
    |                | 4     | 10.4.1.0/24       |  10.4.0.0/16      | Link's                                |
    |                | 4     | 10.4.2.0/24       |  10.4.0.0/16      | MGMT коммутаторов                     |
    |                | 4     | 10.4.4.0/24       |  10.4.0.0/16      | Серверный сегмент                     |
    |                | 4     | 10.200.0.0/24     |  10.200.0.0/24    | Сеть для построения тоннелей          |
    |                | 4     | 100.100.100.0/24  |  100.100.100.0/24 | NAT и сеть Интернет                   |
    |----------------|-------|-------------------|-------------------|---------------------------------------|
    | Добрая Воля    | 5     | 10.6.0.0/24       |  10.6.0.0/16      | Loopback's                            |
    |                | 5     | 10.6.1.0/24       |  10.6.0.0/16      | Коммтаторы Vlan2                      |
    |                | 5     | 10.6.5.0/24       |  10.6.0.0/16      | Пользователи Vlan5                    |
    |                | 5     | 10.6.6.0/24       |  10.6.0.0/16      | Пользователи Vlan6                    |
    |                | 5     | 10.6.7.0/24       |  10.6.0.0/16      | Пользователи Vlan7                    |
    |                | 5     | 10.6.10.0/24      |  10.6.0.0/16      | Сеть для связи с удаленным офисом     |
    |                | 5     | 10.6.20.0/24      |  10.6.0.0/16      | Сеть для связи с центральным филиалом |
    |                | 5     | 10.100.0.0/24     |  10.100.0.0/24    | Сеть для построения тоннелей          |
    |                | 5     | 10.200.0.0/24     |  10.200.0.0/24    | Сеть для построения тоннелей          |
    |----------------|-------|-------------------|-------------------|---------------------------------------|
    | Потерянный Рай | 6     | 192.168.4.0/24    |                   | Loopback's                            |
    |                | 6     | 192.168.4.0/24    |                   | Loopback's                            |
    |                | 6     | 192.168.4.0/24    |                   | Loopback's                            |
    |                | 6     | 192.168.4.0/24    |                   | Loopback's                            |
    |                | 6     | 192.168.4.0/24    |                   | Loopback's                            |
    |                | 6     | 192.168.4.0/24    |                   | Loopback's                            |
